...
exports.addProduct = (req, res) => {
    const { productName, productDescription, productPrice, productStock, categoryId, size } = req.body;
    let productImage;
    if (req.file) {
    ...
    const sql = 'INSERT INTO products (productName, productDescription, productImage, productPrice, productStock, categoryId, size) VALUES (?, ?, ?, ?, ?, ?, ?)';

    // Insert the new product into the database
    db.query(sql, [productName, productDescription, productImage, productPrice, productStock, categoryId, size], (error, results) => {
        if (error) {
...
exports.editProduct = (req, res) => {
    ...
    const sql = 'UPDATE products SET productName = ? , productDescription = ?, productImage = ?, productPrice = ?, productStock = ?, categoryId = ?, size = ?, WHERE productId = ?';

    // Updated the product into the database
    db.query(sql, [productName, productDescription, productImage, productPrice, productStock, categoryId, productId, size], (error, results) => {
        if (error) {
            // Handle any error that occurs during the database operation
